...
export default function AppMap({ position, zoom, name }: MapData) {
    const mapRef2 = useRef<Mapa | null>(null);
    const markerRefs = useRef<Record<string, React.RefObject<LeafletMarker>>>({});

    const SetMapRef2 = () => {
        const map = useMap();
        mapRef2.current = map;
        console.log(mapRef2.current)
        return null;
    };
    ...
    useEffect(() => {
        if (mapRef2.current && name) {
            const universityLocation = getUniversityLocation(name); // Obtén la ubicación de la universidad de alguna manera
            mapRef2.current.flyTo(universityLocation, zoom, { animate: true, duration: 3 }); // Mueve el mapa a la ubicación de la universidad
    ...
    return (
        <div>
            <MapContainer center={position} zoom={zoom} style={{ height: '900px', width: '100%' }} zoomControl={false}>
                <SetMapRef2 />
                <TileLayer
